...
- Classify ordinary continuation as `preparation`, `evidence_collection`, or `candidate_defect`; a defect binds its exact candidate and failure evidence. A matching, revalidated local action may return only a source-Chief local intent. It suppresses TODO escalation; delivery/ACK is not execution or TESTING_GATE_PASS. Visual choices still go only to the Creative Director.
- Relay an already-approved nonvisual operator decision once by stable ID and exact original words to its registered source Chief, while recording a mandatory asynchronous nonblocking General Office audit. Unknown/stale/duplicate IDs do not relay; the receipt is not execution, permission, or Testing evidence. Visual decisions remain Creative-Director-only.

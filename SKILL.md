...
4. New ordinary Chiefs use `pin_primary_task=false`; being unpinned is not a failure and does not trigger a successor. Mandatory pins are limited to `general_office`, `todo`, `creative_director`, and `context_migration_monitor`. Optional product Chief appointment, creation, pin/unpin, replacement, and inheritance require a general-office recommendation followed by the operator's explicit approval. Protect manual non-Chief pins; at full observed capacity provide only a paired replacement recommendation. Approval to appoint or pin never confirms the goal or bypasses the Product Manager discovery gate. For an eligible mandatory or approved lineage, a pin receipt is not proof: require the exact ID in a fresh `list_threads.pinnedThreads` result. Only after a safe same-lineage handoff and `MIGRATION_READY` may exactly one replacement be created, independently pin-verified before takeover and predecessor archival. Read [references/pin-inheritance-governance.md](references/pin-inheritance-governance.md).

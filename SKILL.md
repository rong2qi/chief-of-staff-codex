...
4. `pin_primary_task` is `true` for every Chief, and the active Chief must remain pinned. Resolve the current task ID and pin that task after the rename. Prefer a runtime-provided current task ID; otherwise list tasks and require one exact `primary_task_title` match in the current project context. Never pin an ambiguous match. A successful pin-tool response is not sufficient evidence: immediately call `list_threads` and require the exact task ID to appear in `pinnedThreads` before reporting success. If pinning is unavailable or the independent list check fails, report and record `pin_verification_failed` while leaving project state intact. Do not create a duplicate Chief while the original is active. At the next safe handoff boundary after its current authorized work is complete, archive the original with reason `unable_to_pin`, create exactly one replacement Chief in the same saved project and work state, transfer the current goal/phase/pending approvals and TODOs/write ownership/evidence handoff, and verify the replacement's exact ID in `pinnedThreads` before it takes control. Never delete the original, change scope or pause state, or use this fallback to bypass approval gates. Read [references/pin-inheritance-governance.md](references/pin-inheritance-governance.md).
